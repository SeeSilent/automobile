  ...
  <div>
    ...
    >
      ...
      <el-table-column prop="region" label="汽车系列">
        ...
        <el-table-column prop="_id" label="订单ID" sortable width="180">
      </el-table-column>
      ...
      <template #default="scope">
        <el-button
          @click.prevent="faHuoDing(scope.$index, list)"
          type="text"
          size="small"
        >
          发货
...
export default {
  ...
  setup() {
    const list = ref([]);
    axios.post("http://localhost:3000/gettargetdin", {
        DindanState:"未发货"
    }).then((res) => {
      list.value = res.data
    ...
    const deleteRow = (index, rows) => {
     ...
     const faHuoDing = (index, rows) => {
        console.log(rows[index].userID)
      axios
        .post("http://localhost:3000/fahuodingdan", {
          _id: rows[index]._id,
        })
        .then((res) => {
          if (res.data.erron == "0") {
    ...
    return { list,deleteRow ,faHuoDing};
  },
  components:{
